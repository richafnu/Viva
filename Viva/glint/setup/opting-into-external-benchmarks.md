...
**For an initial survey**, when no historical data exists, most organizations are interested in seeing how their scores compare to an external benchmark. The external benchmark is useful for company admins to provide a meaningful reference point in assessing an organization's overall scores. External benchmarks are helpful initially, but as you begin to survey more frequently, internal trends and comparisons become more useful for making incremental improvements. 
...
>[!TIP]
>There can't be a scenario where consent toggle is Off and an external benchmark is selected. Similarly, there can't be a scenario where consent toggle is On and no external benchmarks are selected.
