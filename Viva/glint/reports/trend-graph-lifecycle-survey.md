...
**For the Executive Summary Report:** If program settings allow users to submit multiple surveys for an ELC survey, they may have multiple response records in raw survey response exports. **However, aggregated reports in the platform count them only once**.
...
> - A unique user submits surveys and has a different department value for each submission:
